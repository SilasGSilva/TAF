...
import { PenAlim, InfoDep, TpRendEnum, TpRendLabelEnum, TpCREnum } from '../../../../../../models/labor-process-taxes.model';
...
export interface PenAlimView {
  cpfDepLabel: string;
  tpRendLabel: string;
  vlrPensaoLabel: number;
}

@Component({
  selector: 'app-alimony-information',
  templateUrl: './alimony-information.component.html',
  styleUrls: [
    './alimony-information.component.scss',
    '../../../labor-process-tax-detail.component.scss',
  ]
})
export class AlimonyInformationComponent implements OnInit, OnDestroy {
  ...
  get formArray(): UntypedFormControl {
    return this.currentFormGroup?.get('penAlim') as UntypedFormControl;
  }

  get formArrayValue(): PenAlim[] {
  ...
  get errorMessageTpRend(): string {
    const errors = this.subFormGroup.get('tpRend')?.errors;
    if (!errors) {
      return '';
    }
    ...
    if (errors.valueMustBeRra) {
      return 'A opção selecionada deve ser "RRA" já que "Código de Receita" é "IRRF - RRA"';
    }
  }

  get offsetErrorMessageTpRend(): string {
    if (this.errorMessageCpfDep) {
      return '';
    }

    return 'po-offset-sm-4 po-offset-md-4 po-offset-lg-4 po-offset-xl-4'
  }
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.push(this.currentFormGroup.get('tpCR').valueChanges.subscribe(() => {
      this.updateValidityTpRend();
    }));

    this.subscriptions.push(this.subFormGroup.get('tpRend').valueChanges.subscribe(() => {
      this.updateValidityTpRend();
    }));
  ...
  createSubFormGroup(): UntypedFormGroup {
    return this.fb.group({
      ...
      vlrPensao: [null, Validators.min(0.01)],
    });
  }
  ...
  transformRows(): void {
    this.rows = this.formArrayValue.map(penAlim => ({
      cpfDepLabel: FormatterUtils.formatCpf(penAlim.cpfDep),
      tpRendLabel: FormatterUtils.convertValueToLabel(penAlim.tpRend, TpRendEnum, TpRendLabelEnum),
      vlrPensaoLabel: penAlim.vlrPensao,
    }));
  }
  ...
  handleClickEdit(penAlim: PenAlimView): void {
    const clickIndex = this.rows.findIndex(item => item === penAlim);
  ...
  handleClickDelete(penAlim: PenAlimView): void {
    const clickIndex = this.rows.findIndex(item => item === penAlim);
  ...
  private updateValidityTpRend(): void {
    const tpCR: TpCREnum = this.currentFormGroup.get('tpCR').value;
    const currentField = this.subFormGroup.get('tpRend');
    const errors = tpCR === TpCREnum.IRRFRRA && currentField.value !== TpRendEnum.rra
      ? { valueMustBeRra: true }
      : null;
